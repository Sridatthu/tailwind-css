...
export default function Home() {
  ...
  return (
    <div className="relative h-full w-full flex items-center justify-center">
      <Pattern/>
      <div className="relative z-10 h-80 w-60 dark:bg-red-600 bg-neutral-200 shadow-2xl border border-neutral-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-center gap-4 h-40 mask-r-from-50% mask-l-from-50% animate-marquee">
          {icons.map((item,index)=>(<div 
          className={`rounded-full bg-neutral-300 flex items-center justify-center size-10`}
          key={item.title}>{item.icon}</div>))}
        </div>
      </div>
    </div>
  );
